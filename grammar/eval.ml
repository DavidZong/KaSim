...
let volume_of_ast abs_vol env =
	let ((name, pos),vol_siz,(param,pos_p)) = abs_vol in
	let p =
		match param with
			| "active" -> 0
			| "passive" -> 1
			| "sink" -> 2
			| _ as s -> raise (ExceptionDefn.Syntax_Error ((Some pos_p),Printf.sprintf "Volume control '%s' is undefined" s))
	in
	Environment.declare_volume name pos vol_siz p env
...
let environment_of_result res =
	...
	in
	List.fold_left (fun env volume_dec -> volume_of_ast volume_dec env) env res.Ast.volumes
...
let pert_of_result variables env res =
	let (variables, lpert, lrules, env) =
		List.fold_left
			(fun (variables, lpert, lrules, env) (bool_expr, vol_opt, modif_expr_list, pos, opt_post) ->				
				let location = 
					match vol_opt with 
						None -> None 
					| Some (str,pos) -> 
						try Some (Environment.num_of_volume str env) with Not_found -> 
							raise	(ExceptionDefn.Semantics_Error (pos,"Compartment "^str^" is undeclared"))
				in 
...
let init_graph_of_result env res =
	...
	in
	(*Creating scheduler for active volumes*)
	(*let sched = Scheduler.
	List.iter
	() res.Ast.volumes*)  
	(sg_map,env)
...
let initialize result counter =
		...
		(fun vol_num _ vol_map ->
			...
			if volume_control > 0 then 
				...
				and filtered_pert = 
					List.filter (*leaving only rules which may be applied in the volume (none if sink)*)
  				(fun p -> 
  					match p.located with 
  					| Some vol_num' -> (vol_num' = vol_num) 
  					| None -> vol_num=0 (*Rule with no diffusion annotation may only occur at toplevel*)
  				) pert
				and filtered_rule_pert =
					List.filter (*leaving only rules which may be applied in the volume (none if sink)*)
  				(fun r -> 
  					match r.location with 
  					| Some vol_num' -> (vol_num' = vol_num) 
	...
	let vol_id = ref 0
	and ptr_state_map = ref IntMap.empty
	and ptr_env = ref env
	in
	...
	while !vol_id < env.Environment.fresh_volume_id do
		...
		let (state, new_env) =
			let diffusion_rules =
				try
					let diffusion_rules,_,_,_,pert,rule_pert = IntMap.find volume_num vol_map 
					in
					diffusion_rules
				with Not_found -> []
			in
